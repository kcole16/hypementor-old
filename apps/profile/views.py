...
from apps.profile.utils import authenticate_linkedin, upload_file_to_dropbox, \
    get_linkedin_profile, connect_mongodb
...
def home(request):
    ...
    db = connect_mongodb()
    client_code = db.authorized.find_one({'linkedin_id': profile.linkedin_id})[
        'client_code']
...
def dashboard(request):
    users = User.objects.raw(
        "select * from auth_user au inner join profile_profile pp on pp.user_id = au.id inner join profile_interest pi on pi.user_id = au.id inner join profile_resume pr on pr.user_id = au.id ")
    return render_to_response('profile/dashboard.html', {'users': users},
...
def submit(request):
    ...
    if request.POST:
        ...
        form = SubmitForm()
    return render_to_response('profile/submit.html',
                              {'form': form, 'upload_error': upload_error,
...
def oauth(request):
    ...
    if authorized == None:
        return render_to_response('profile/unauthorized.html',
                                  context_instance=RequestContext(request))
    else:
